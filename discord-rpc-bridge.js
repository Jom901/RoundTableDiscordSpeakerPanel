...
if (!CLIENT_ID || !CLIENT_SECRET) {
  ...
  console.error('[Bridge]   DISCORD_CLIENT_ID=1489607111531958273 \\');
...
class FrameParser {
  ...
  push(chunk) {
    ...
    while (this.buf.length >= 8) {
      ...
      try { frames.push({ opcode, data: JSON.parse(raw) }); }
      catch { console.warn('[IPC] Unparseable frame:', raw); }
    }
    return frames;
  }
}
...
wss.on('connection', (ws, req) => {
  wsClients.add(ws);
  console.log(`[Bridge] Browser connected (${req.socket.remoteAddress}), total: ${wsClients.size}`);
  ws.on('close', () => {
    wsClients.delete(ws);
...
function rpcSubscribe(evt, args = {}) {
  return new Promise((resolve, reject) => {
    ...
    ipcSocket.write(encodeFrame(OP.FRAME, { cmd: 'SUBSCRIBE', evt, args, nonce }));
  });
}
...
async function subscribeToChannel(channelId) {
  ...
  console.log('[RPC] Subscribed to SPEAKING_START / SPEAKING_STOP / VOICE_STATE_CREATE / VOICE_STATE_DELETE');
}
...
async function syncVoiceChannel() {
  try {
    ...
    const ch  = res?.data;
    if (ch?.id) {
      const members = (ch.voice_states ?? []).map(vs => ({ ...vs.user, nick: vs.nick ?? null })).filter(Boolean);
      console.log(`[RPC] Currently in voice channel: ${ch.name} (${ch.id}), members: ${members.length}`);
...
async function doAuth() {
  try {
    ...
    console.log('[Auth] Sending AUTHORIZE (Discord will show an approval modal)...');
    ...
    console.log('[Auth] Access token obtained (expires in', tokenData.expires_in, 's)');

    // Step 3: Authenticate with the token
    const authData = await rpcCall('AUTHENTICATE', { access_token: tokenData.access_token });
...
function handleFrame({ opcode, data }) {
  ...
  if (evt === 'READY') {
    ...
    broadcast({ type: 'CONNECTION', status: 'connected', v: d?.v, config: d?.config, user: d?.user });
  ...
  if (evt === 'SPEAKING_START') {
    ...
    broadcast({ type: 'SPEAKING_START', user_id: d?.user_id, channel_id: d?.channel_id });
    return;
  }

  if (evt === 'SPEAKING_STOP') {
    console.log('[RPC] SPEAKING_STOP   user_id:', d?.user_id);
    broadcast({ type: 'SPEAKING_STOP', user_id: d?.user_id, channel_id: d?.channel_id });
    return;
  }
  ...
  if (evt === 'VOICE_CHANNEL_SELECT') {
    ...
    console.log('[RPC] VOICE_CHANNEL_SELECT  channel_id:', channelId ?? '(none)');
